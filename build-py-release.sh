...
meson compile -C python/lib/build-release -j $NJOBS && MESON_NUM_PROCESSES=$NJOBS meson install -C python/lib/build-release && MESON_NUM_PROCESSES=$NJOBS meson test -C python/lib/build-release
